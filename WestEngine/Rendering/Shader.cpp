...
namespace WestEngine
{
	...
	Shader::Shader(const std::string& vertexPath, const std::string& fragmentPath)
	{
		...
		// Read shaders
		std::string vertexShaderSource = Helper::ReadAllText(Helper::AssetsPath().append(vertexPath).generic_string<char>());
		std::string fragmentShaderSource = Helper::ReadAllText(Helper::AssetsPath().append(fragmentPath).generic_string<char>());
		...
		const char* source1 = fragmentShaderSource.c_str();
		...
		glShaderSource(fragmentShader, 1, &source1, NULL);
	...
	void Shader::setVec2(const std::string& name, const 
		
		
		vec2& value)  { glUniform2fv(map[name].id, 1, &value[0]); }
	void Shader::setVec3(const std::string& name, const Math::vec3& value)  { glUniform3fv(map[name].id, 1, &value[0]); }
	...
	void Shader::setVec4(const std::string& name, const Math::vec4& value) { glUniform4fv(map[name].id, 1, &value[0]); }
	void Shader::setMat2(const std::string& name, const Math::mat2& mat) { glUniformMatrix2fv(map[name].id, 1, GL_FALSE, &mat[0][0]); }
	void Shader::setMat3(const std::string& name, const Math::mat3& mat) { glUniformMatrix3fv(map[name].id, 1, GL_FALSE, &mat[0][0]); }
	void Shader::setMat4(const std::string& name, const Math::mat4& mat) { glUniformMatrix4fv(map[name].id, 1, GL_FALSE, &mat[0][0]); }
